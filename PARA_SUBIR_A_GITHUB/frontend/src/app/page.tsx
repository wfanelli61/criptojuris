...
const mockTestimonials: Testimonial[] = [
    { id: '1', name: 'Roberto Hernández', role: 'Director General', rating: 5, content: 'Contratamos a BufeteLegal para una reestructuración corporativa compleja. El equipo dominó cada detalle legal y nos explicó todo en lenguaje claro. Resolvieron todo en 3 meses.' },
    { id: '2', name: 'Carmen Díaz', role: 'Emprendedora', rating: 5, content: 'Tenía un conflicto laboral imposible. La Dra. Rodríguez estudió mi caso a fondo, me explicó mis opciones y logró un acuerdo favorable sin ir a juicio.' },
];
                    ...
                    <PhysicsCarousel itemWidth={340}>
                        {lawyers.map((lawyer, i) => (
                            <div key={lawyer.id} className="physics-carousel-item" style={{
                                background: `linear-gradient(160deg, rgba(27,59,90,0.95), rgba(16,37,64,0.98))`, border: `1px solid rgba(255,255,255,0.1)`, borderRadius: '1.25rem', padding: '2rem 1.5rem', textAlign: 'center', backdropFilter: 'blur(20px)', position: 'relative', overflow: 'hidden', boxShadow: '0 12px 40px rgba(0,0,0,0.3)', display: 'flex', flexDirection: 'column', minHeight: '480px'
                            }}>
                                ...
                                <div style={{
                                    width: '80px', height: '80px', borderRadius: '50%', background: `linear-gradient(135deg, ${C.yellow}, ${C.orange})`, display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0.5rem auto 1.2rem', fontSize: '1.4rem', color: C.navyDeep, fontWeight: 800, boxShadow: `0 6px 20px ${C.yellow}30`, border: '3px solid rgba(255,255,255,0.15)',
                                }}>{(lawyer.name || 'A B').split(' ').map(n => n[0]).slice(0, 2).join('')}</div>
                                ...
                                {lawyer.lawyerProfile && (
                                    <>
                                        <p style={{ fontSize: '0.82rem', color: C.textMuted, marginBottom: '0.9rem' }}>📍 {lawyer.lawyerProfile.city} · {lawyer.lawyerProfile.yearsExperience} años exp.</p>
                                        <div style={{ display: 'flex', gap: '0.4rem', justifyContent: 'center', flexWrap: 'wrap', marginBottom: '1.2rem' }}>
                                            {(lawyer.lawyerProfile.specialties || []).map(s => (
                                                <span key={s} style={{ background: 'rgba(255,255,255,0.08)', color: C.white, padding: '0.25rem 0.7rem', borderRadius: 'var(--radius-full)', fontSize: '0.72rem', fontWeight: 500, border: '1px solid rgba(255,255,255,0.12)', }}>{s}</span>
                                            ))}
                                        </div>
                                        <div style={{ height: '1px', background: 'rgba(255,255,255,0.08)', margin: '0 0 1rem' }} />
                                        <p style={{ fontSize: '1rem', fontWeight: 600, marginBottom: '1.25rem', color: C.white, }}>Desde <span style={{ color: C.yellow, fontWeight: 900, fontSize: '1.25rem', textShadow: `0 0 10px ${C.yellow}40` }}>${lawyer.lawyerProfile.ratePerHour}</span>/hora</p>
                                    </>
                                )}
                                <Link href={`/abogados/${lawyer.id}`} className="btn btn-sm" style={{ width: '100%', background: `linear-gradient(135deg, ${C.yellow}, ${C.orange})`, color: C.navyDeep, fontWeight: 700, border: 'none', boxShadow: `0 4px 16px ${C.yellow}30`, marginTop: 'auto' }}>Ver Perfil y Agendar →</Link>
                            </div>
                        ))}
                    </PhysicsCarousel>
                </div>
            ...
            <section id="faq" className="page-section" style={{ position: 'relative', overflow: 'hidden', background: C.navyDark, scrollMarginTop: '15vh' }}>
                <div className="container" style={{ maxWidth: '800px' }}>
                    ...
                    <div className="stagger-children" style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                        {[
                            { q: '¿La primera consulta tiene algún costo?', a: 'No. Es completamente gratuita y sin compromiso.' },
            ...
            <section style={{ padding: '4rem 1rem', position: 'relative', overflow: 'hidden', textAlign: 'center', background: C.navyDeep }}>
                <div className="container reveal-bounce" style={{ maxWidth: '700px', position: 'relative', zIndex: 1 }}>
                    <h2 style={{ fontSize: 'clamp(1.8rem, 4vw, 2.8rem)', marginBottom: '1.25rem', color: C.white, fontFamily: 'var(--font-heading)' }}>
                        ...
                        <span style={{ background: `linear-gradient(135deg, ${C.yellowBright}, ${C.yellow}, ${C.orange})`, WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', }}>Dé el primer paso hoy.</span>
                    </h2>
                    <div style={{ display: 'flex', gap: '1rem', justifyContent: 'center', flexWrap: 'wrap', marginTop: '2.5rem' }}>
                        <Link href="/abogados" className="btn btn-lg" style={{ background: `linear-gradient(135deg, ${C.yellow}, ${C.orange})`, color: C.navyDeep, fontSize: '1.05rem', boxShadow: `0 8px 32px ${C.yellow}50`, border: 'none', fontWeight: 800, }}>🗓️ Agendar Consulta Gratuita</Link>
                        <Link href="/registro" className="btn btn-lg" style={{ background: `${C.yellow}15`, color: C.yellow, border: `2px solid ${C.yellow}35`, backdropFilter: 'blur(8px)', }}>Crear Cuenta Gratis</Link>
                    </div>
                </div>
            </section>
        </>
