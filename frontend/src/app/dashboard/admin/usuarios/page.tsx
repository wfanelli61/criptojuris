...
export default function AdminUsuariosPage() {
 ...
 <h1 style={{ fontSize: '1.6rem', color: '#F1F5F9' }}>Gestión de Usuarios</h1>
 ...
 {[
 { label: 'Total Usuarios', value: metrics.totalUsers, accent: '#60A5FA' },
 { label: 'Abogados', value: metrics.totalLawyers, accent: '#F0B429' },
 { label: 'Clientes', value: metrics.totalClients, accent: '#34D399' },
 { label: 'Citas Pendientes', value: metrics.pendingAppointments, accent: '#F87171' },
 { label: 'Servicios', value: metrics.totalServices, accent: '#C084FC' },
 ].map((m) => (
 <div key={m.label} style={{ background: 'rgba(255,255,255,0.04)', border: `1px solid ${m.accent}30`, padding: '1.25rem', borderRadius: '1rem', textAlign: 'center' }}>
 <div style={{ fontSize: '1.8rem', fontWeight: 700, color: m.accent }}>{m.value}</div>
 <div style={{ fontSize: '0.8rem', color: 'rgba(255,255,255,0.45)', marginTop: '0.25rem' }}>{m.label}</div>
 </div>
 ))}
 </div>
 )}
 ...
 {message && (
 <div style={{ padding: '0.75rem', background: '#D1FAE5', borderRadius: 'var(--radius-md)', color: '#34D399', fontSize: '0.85rem', marginBottom: '1rem' }}>
 {message}
 ...
 {/* Filters */}
 <form onSubmit={handleSearch} className="dash-filters">
 ...
 <thead>
 <tr><th>Nombre</th><th>Email</th><th className="col-hide-mobile">Teléfono</th><th>Rol</th><th className="col-hide-mobile">Creado</th><th>Acciones</th></tr>
 </thead>
 <tbody>
 {users.map((u) => (
 <tr key={u.id}>
 ...
 <td>{u.email}</td>
 <td className="col-hide-mobile">{u.phone || '—'}</td>
 <td>
 ...
 <td className="col-hide-mobile" style={{ fontSize: '0.85rem' }}>{new Date(u.createdAt).toLocaleDateString('es-ES')}</td>
