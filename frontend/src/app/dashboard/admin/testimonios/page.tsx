...
export default function AdminTestimoniosPage() {
 ...
 const [form, setForm] = useState({ name: '', role: '', rating: 5, content: '', active: false });
 ...
 const resetForm = () => {
 setForm({ name: '', role: '', rating: 5, content: '', active: false });
 setEditId(null);
 setShowForm(false);
 };
 ...
 setForm({ name: t.name, role: t.role || '', rating: t.rating, content: t.content, active: t.active });
 setEditId(t.id);
 setShowForm(true);
 };
 ...
 <h1 style={{ fontSize: '1.6rem', color: '#F1F5F9' }}>Gestión de Testimonios</h1>
 ...
 <div className="form-group"><label className="form-label">Nombre del cliente</label><input className="form-input" value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} required /></div>
 <div className="form-group"><label className="form-label">Rol/Profesión</label><input className="form-input" value={form.role} onChange={(e) => setForm({ ...form, role: e.target.value })} /></div>
 ...
 <input type="checkbox" id="approved" checked={form.active} onChange={(e) => setForm({ ...form, active: e.target.checked })} />
